    ...
    <div className="w-full flex flex-col">
      ...
      <div className="flex flex-col">
        {players.slice(0, 10).map((player) => (
          <div key={player.id} className={`grid grid-cols-[40px_1fr_auto] md:grid-cols-[70px_1fr_auto] items-center gap-2 md:gap-4 px-3 md:px-6 py-2.5 md:py-4 transition-all duration-300 ${getRankClasses(player.rank)} ${player.rank > 3 ? '' : 'my-[2px] md:my-1 rounded-xl'}`}>
            <div className="flex justify-center items-center">
              {player.rank === 1 && <span className="text-xl md:text-2xl drop-shadow-[0_0_12px_rgba(251,227,163,0.9)]">👑</span>}
            ...
            <div className={`font-hero font-bold tracking-wide text-[13px] md:text-base ${player.rank === 1 ? 'text-brand-gold' : 'text-brand-offwhite'}`}>
              {player.username}
            </div>
            <div className={`font-hero font-bold tabular-nums text-[13px] md:text-base ${player.rank === 1 ? 'text-brand-gold' : 'text-brand-offwhite'}`}>
